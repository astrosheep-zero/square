import { activeListeners, extractMentions, formatActivityId, parseActivityId, type ActivityId } from './square-core.js';
...
import { deriveDeliveryModel, isActivitySeen, perceiveActivity, planActNotifications, type PlannedNotification } from './delivery.js';
import { SquareError, nameKey, type ActivitiesOptions, type InboxNotification, type PublicAct, type RoomChangeAct, type SquareState, type StoredAct } from './model.js';
...
function exposePerceived(state: Parameters<typeof perceiveActivity>[0], stored: StoredAct, viewer: string): PerceivedActivity {
...
export async function currentParticipant(square: OpenSquare, name: string): Promise<string | undefined> { const { state } = await square.cell.read(); const known = resolveRosterName(state, name); return known !== undefined && isCurrentlyJoined(state.acts, known) ? known : undefined; }
export async function admitsBareExpress(square: OpenSquare, name: string, body: string): Promise<boolean> { const { state } = await square.cell.read(); const actor = resolveKnownName(state, name); return extractMentions(body).length > 0 || activeListeners(foldedState(state), actor).length > 0; }
...
export async function historyPresentation(square: OpenSquare, options: ActivitiesOptions): Promise<HistoryPresentation> { const { state } = await square.cell.read(); return { activities: coreActivities(state, options).map((activity) => ({ ...activity, perception: options.viewer === undefined ? 'full' as const : perceiveActivity(state, activity, options.viewer) })), sayNumbers: sayNumbers(state), presenceAnchors: anchors(state), participantCount: inSquareCount(state) }; }
...
export async function statusPresentation(square: OpenSquare): Promise<StatusPresentation> { const { state } = await square.cell.read(); const status = coreStatus(state, square.clock()); return { state, status, ...(status.latestAct?.kind === 'say' ? { latestActNumber: countSays(state.acts, status.latestAct.actor) } : {}) }; }
