...
export interface HostLedgerPort { claimPresence(input:PresenceRecord, scope?:HostLedgerScope):Promise<PresenceClaimResult>; ensurePresence(input:PresenceRecord, scope?:HostLedgerScope):Promise<PresenceResult>; removePresence(input:PresenceKey):Promise<void>; listPresence(input:PresenceLookup):Promise<readonly PresenceRecord[]>; claimEvidence(input:EvidenceClaim):Promise<ClaimResult>; releaseEvidence(input:EvidenceRelease):Promise<void>; appendEvidence(input:EvidenceRecord & { readonly claimToken: string }):Promise<void>; listEvidence(input:EvidenceLookup):Promise<readonly EvidenceRecord[]>; listWakeAttempts(input?:WakeAttemptLookup):Promise<readonly EvidenceRecord[]>; appendWakeAttempt(input:EvidenceRecord & { readonly claimToken: string }):Promise<void>; claimWakeDispatch(input:WakeDispatchClaimInput):Promise<WakeDispatchClaim>; transitionWakeDispatch(input:WakeDispatchTransitionInput):Promise<boolean>; releaseWakeDispatch(input:WakeDispatchReleaseInput):Promise<void>; gcEvidence(input:EvidenceGc):Promise<void>; reconcileBinding(input:ReconcileBindingInput):Promise<ReconcileBindingResult> }
