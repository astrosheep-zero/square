...
test('every reach mode addresses exactly its eligible peers', () => {
  const square = doc([
    ...
    { kind: 'say', actor: 'Alice', at: 6, body: 'private', reach: { beside: 'cara' } },
    { kind: 'say', actor: 'Alice', at: 7, body: 'attention', reach: 'bell' },
  ]);
  const delivery = deriveDeliveryModel(square);

  assert.deepEqual(plannedRecipients(delivery, square.acts[3]), ['Bob:broadcast', 'Cara:broadcast']);
  assert.deepEqual(plannedRecipients(delivery, square.acts[4]), ['Bob:mention']);
  assert.deepEqual(plannedRecipients(delivery, square.acts[5]), ['Cara:beside']);
...
test('a catch lease owns only the notifications admitted by its filter', () => {
  ...
  const beside = { actor: 'Alice', body: 'private', route: 'beside', recipient: 'Bob' };
  ...
  assert.equal(leaseOwnsNotification({ ...lease, filter: { mention: 'Bob' } }, beside), true);
  assert.equal(leaseOwnsNotification({ ...lease, filter: { mention: 'Cara' } }, beside), false);
});
...
test('pending projection remains complete across a long history', () => {
  ...
  for (let index = 0; index < 4_000; index++) {
    acts.push({
      ...
      actor: 'Alice',
      at: index + 3,
      body: index % 1_000 === 0 ? `direct ${index} @Bob` : `broadcast ${index}`,
    });
  }
